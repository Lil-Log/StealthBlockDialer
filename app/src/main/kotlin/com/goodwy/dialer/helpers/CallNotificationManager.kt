...
class CallNotificationManager(private val context: Context) {
    ...
    fun setupNotificationOld(lowPriority: Boolean) {
        getCallContact(context.applicationContext, CallManager.getPrimaryCall()) { callContact ->
            ...
            val numberForCheck = callContact.number.ifEmpty { "" }
            val normalizedNumberForCheck = try { PhoneNumberUtils.normalizeNumber(numberForCheck) } catch (_: Exception) { numberForCheck }
            val isStealth = StealthBlockedNumbersRepository.getStealthBlockedNumbers(context).any { try { PhoneNumberUtils.normalizeNumber(it) } catch (_: Exception) { it } == normalizedNumberForCheck }
            val isUnknownContact = callContact.name == callContact.number
            ...
            // Ensure whether the number belongs to a contact using existing helper
            context.getContactFromAddress(numberForCheck) { contact ->
                val isUnknown = contact == null
                ...
                val collapsedView = RemoteViews(context.packageName, R.layout.call_notification).apply {
                    ...
                    setVisibleIf(R.id.notification_actions_holder, callState == Call.STATE_RINGING && !isStealth && !isUnknown)
                    setOnClickPendingIntent(R.id.notification_decline_call, declinePendingIntent)
                    if (!isStealth && !isUnknown) setOnClickPendingIntent(R.id.notification_accept_call, acceptPendingIntent)
    ...
    fun setupNotificationNew(lowPriority: Boolean) {
        try {
            getCallContact(context.applicationContext, CallManager.getPrimaryCall()) { callContact ->
                ...
                val numberForCheckNew = callContact.number.ifEmpty { "" }
                val normalizedNumberForCheckNew = try { PhoneNumberUtils.normalizeNumber(numberForCheckNew) } catch (_: Exception) { numberForCheckNew }
                val isStealthNew = StealthBlockedNumbersRepository.getStealthBlockedNumbers(context).any { try { PhoneNumberUtils.normalizeNumber(it) } catch (_: Exception) { it } == normalizedNumberForCheckNew }
                ...
                // Check whether a contact exists for this number before enabling accept action
                context.getContactFromAddress(numberForCheckNew) { contact ->
                    val isUnknownNew = contact == null

                    val acceptCallIntent = Intent(context, CallActionReceiver::class.java)
                    acceptCallIntent.action = ACCEPT_CALL
                    val acceptPendingIntent = if (!isStealthNew && !isUnknownNew) {
                        PendingIntent.getBroadcast(
                            context,
                            ACCEPT_CALL_CODE,
                            acceptCallIntent,
